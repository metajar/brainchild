...
export default function Brainchild() {
  ...
  const kitSpecs = () => {
    ...
    const writeBack = `WRITE-BACK IS MANDATORY. A task is not done until context is current. After every completed task — and always after a major task (feature, architecture change, API/data-model change, infra, or anything that invalidates prior assumptions) — the agent MUST: (1) append a tight entry to .mex/context/changelog.md (what changed, why, files touched, follow-ups); (2) update every .mex/ file that the work made stale (architecture, stack, setup, decisions, conventions, patterns); (3) add a decisions.md entry when a new choice was made; (4) add or update a patterns entry when a reusable approach was discovered. Do not leave the wiki behind the code.`;
    return [
      { path: a.path, spec: `The project anchor file for ${a.label}. Small and always-loaded: 1-2 sentence project summary, the tech stack in one line, a pointer telling the agent to read .mex/ROUTER.md before any task to load routed context, the 5 most important conventions/commands, and a hard close-out rule: after every completed task (always after a major one) document the change in .mex/context/changelog.md and update any stale .mex/ context files before considering the work finished. Under 70 lines.` },
      { path: ".mex/AGENTS.md", spec: `Instructions for AI coding agents working in this repo: how the .mex wiki is organized, when to read ROUTER.md, and the full write-back protocol. ${writeBack} Include a close-out checklist the agent must run at the end of every task: changelog entry; which context files to revisit; new decision?; new pattern?; ROUTER still accurate? State that skipping write-back is a defect, not optional cleanup.` },
      { path: ".mex/ROUTER.md", spec: `The context router. Two tables: (1) LOAD — task-type → which .mex/context/*.md and patterns files to read before starting (e.g. new feature, bug fix, API change, UI work, data model change, infra); (2) WRITE-BACK — the same task types → which files to update after the work lands. Always-load line, default route, and a rule that write-back is part of finishing the task, not a follow-up.` },
      { path: ".mex/context/architecture.md", spec: "The system architecture distilled for agent consumption: components and responsibilities, how they communicate, the ASCII component diagram, and the data flow. Derived from the architecture doc; tighter and more declarative. End with a one-line 'Update this file when' rule: any component, boundary, or data-flow change." },
      { path: ".mex/context/stack.md", spec: "The exact tech stack: languages, frameworks, key libraries, datastores, infra — each with a one-line 'why' and any version constraints. End with 'Update this file when' a dependency, version, or infra choice changes." },
      { path: ".mex/context/setup.md", spec: "Bootstrap instructions for a fresh clone: prerequisites, install, env vars (names + purpose, never values), run dev, run tests, build. Where the design hasn't fixed a detail yet, write the intended command and mark it TODO. End with 'Update this file when' install, env, or run commands change." },
      { path: ".mex/context/decisions.md", spec: "An architecture-decision log seeded from the design: each entry is '## D-00N — <title>' with Decision / Alternatives considered / Rationale / Status. Include every decision resolved during the founder Q&A. Instruct agents to append a new D-00N entry whenever a consequential choice is made during implementation, and to mark superseded decisions Status: superseded." },
      { path: ".mex/context/conventions.md", spec: "Code conventions for this project: naming, file/folder layout, error handling, testing expectations, commit style, and product-specific rules implied by the PRD (e.g. how user-facing copy is written). End with 'Update this file when' a convention is added, changed, or dropped in real work." },
      { path: ".mex/context/changelog.md", spec: `The living work log. Seed a short header that states: after every completed task the agent appends an entry here, then updates any other .mex/ files the work made stale. Entry format: ### YYYY-MM-DD — <task title> then bullets for Changed / Why / Context files updated / Follow-ups. Seed one example entry from the initial design (kit generated) so the format is obvious. No empty placeholders.` },
      { path: ".mex/patterns/INDEX.md", spec: "The patterns index, mostly empty at project start: explain what belongs here (reusable implementation patterns discovered during real work), the entry format, and seed 1-2 anticipated pattern stubs based on the architecture. Instruct agents to add a pattern when they invent or reuse a non-obvious approach, and to update the index in the same task — not later." },
    ];
  };
  ...
  const genKit = async () => {
    ...
    try {
      for (let i = 0; i < specs.length; i++) {
        ...
        const text = await callClaude({ apiKey,
          model,
          system: `You generate one file of a mex-style project context kit (github.com/mex-memory/mex): a repo-local Markdown wiki that gives AI coding agents persistent project memory. Write the file so a coding agent (not a human reader) gets maximum signal per token: declarative, specific, no marketing language, no placeholders like "[insert here]". Standing rule for every file: finishing a task includes documenting what changed in .mex/context/changelog.md and updating any stale .mex/ context — write-back is mandatory, not optional. Output ONLY the raw file content — no fences, no commentary.`,
          ...
          <div style={{ ...sheet, maxWidth: 1020 }}>
            ...
            {Object.keys(files).length === 0 && !busy && (
              <div style={{ border: `1.5px dashed ${C.sheetEdge}`, borderRadius: 8, padding: "40px 20px", textAlign: "center", fontFamily: "'Inter', sans-serif", fontSize: 13.5, color: C.inkSoft }}>
                The kit, drafted from your finalized PRD, architecture, and interview answers — including a write-back loop so context stays current as you build.
              </div>
            )}
          </div>
        )}
